...
2. **Clinician Interactive Workspace Dashboard (HTML/JS/CSS)**: A premium, dark-mode web application that runs the agent, provides a live visual step-trace, and lets a clinician review summaries and submit feedback corrections to show dynamic learning.

---

## 1. Agentic Loop Design & Architecture
...
[START]
   │
   ▼
1. Document Ingestion (Reads all pages, extracts programmatic/OCR layers)
   │
   ▼
2. Identity & Demographic Match (Reconciles MRN, dates, and names)
   │
   ▼
3. Diagnostic History Cross-Reference (Checks for past history and note-to-note conflicts)
   │
   ▼
4. Prescriptions Extraction (Pulls ICU, ward, and regular drug prescriptions)
   │
   ▼
5. Medication Reconciliation (Compares inpatient active meds vs discharge advice list)
   │
   ▼
6. Pharmacological Safety Verification (Calls external drug-interaction check tools)
   │
   ▼
7. Dynamic Feedback Lookup (Queries clinician correction memory database)
   │
   ▼
8. Structured Compilation & Safety Badging (Emits final JSON draft + complete trace log)
   │
   ▼
[END]
```
...
## 2. Enforcing the "No-Fabrication" Safety Guardrail
...
$$\text{Edit Burden} = \frac{\text{Levenshtein Edit Distance (Draft, Corrected)}}{\max(\text{Length(Draft)}, \text{Length(Corrected)})}$$
...
1. Open your File Explorer and navigate to the project folder:
   `c:\Users\harsh\Desktop\Discharge Summary AI\web\`
2. Double-click **`index.html`** to open it in your browser.

### Step 3: Run the Demonstration Loop
